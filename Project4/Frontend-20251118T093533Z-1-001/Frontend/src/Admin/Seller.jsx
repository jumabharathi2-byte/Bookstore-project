...
const Seller = () => {
  ...
  const fetchSellers = () => {
   axios.get("http://localhost:4000/api/admin/sellers", { withCredentials: true })
  ...
  const deleteSeller = async (id) => {
    try {
     axios.delete(`http://localhost:4000/api/admin/seller/${id}`, { withCredentials: true })
  ...
  const deleteItem = async (id) => {
    try {
     axios.delete(`http://localhost:4000/api/seller/item/${id}`, { withCredentials: true })
      alert('Item deleted');
...
axios.get(`http://localhost:4000/api/seller/items/${sellerId}`, { withCredentials: true })
      .then((response) => {
     ...
     .catch((err) => {
  console.error(err);
  alert("Failed to fetch seller items.");
});
