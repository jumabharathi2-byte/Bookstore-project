...
const Uitem = () => {
  ...
  const fetchItem = async () => {
    try {
      console.log("Fetching item at:", `http://localhost:4000/user/item/${id}`);
      const resp = await axios.get(`http://localhost:4000/user/item/${id}`);
      setItem(resp.data);
    } catch (err) {
      console.error("Did not get data", err.response?.data || err.message);
    }
  };
